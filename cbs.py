...
def detect_collision(path1, path2):
    ...
    for i in range(1, max_len):
        ...
        # edge
        if node1 == node2p and node2 == node1p:
...
class CBSSolver(object):
    ...
    def __init__(self, starts, goals, heuristics, nodes_dict,time_start, agent_id, currents):
...
def run_CBS(aircraft_lst, nodes_dict, edges_dict, heuristics, t):
    ...
    for id, ac in enumerate(aircraft_lst):
        ...
        if ac.status == "taxiing":
            starts.append(ac.from_to[1])#path_to_goal[0][0])
            goals.append(ac.goal)
            time_starts.append(t+0.5) #Need to finish current movement first
